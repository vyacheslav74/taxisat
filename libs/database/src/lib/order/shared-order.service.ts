...
export class SharedOrderService {
  ...
  async calculateFare(input: {
    ...
    Logger.log(
      `Creating order for userId:${input.riderId} with input: ${JSON.stringify(
        input,
      )}`,
  ...
  async createOrder(input: {
    ...
    Logger.log(
      `Creating order for userId:${input.riderId} with points: ${JSON.stringify(
        input,
      )}`,
      'SharedOrderService.createOrder',
    );
    let zonePricings: ZonePriceEntity[] = [];
    if (input.points.length == 2) {
      zonePricings = await this.getZonePricingsForPoints(
    ...
    Logger.log(`zonePricing: ${JSON.stringify(zonePricing)}`, 'createOrder');
    ...
    Logger.log(
      `Regions for points: ${JSON.stringify(regions)}`,
      'SharedOrderService.createOrder',
    );
    ...
    Logger.log(
      `Order created: ${JSON.stringify(orderObject)}`,
      'SharedOrderService.createOrder',
    );
